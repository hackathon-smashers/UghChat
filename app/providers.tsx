...
const config = {
    ...
    databaseURL: "https://hackathon-smashers-default-rtdb.asia-southeast1.firebasedatabase.app/",
}
...
export const DatabaseProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    // Onload
    useEffect(() => {
        if (status == "authenticated") { // logged in
            _sendMessage("Woo Jin Lee", "6a54sd68a45sd", "Hello World!")
            const presenceRef = ref(db, '.info/connected');

            const user_id = data?.user?.name  // TODO: Replace with actual User ID.
        ...
        } else if (status == "unauthenticated") { //not logged in
        ...
        const _onChatChangeHandler = (data: any) => {
            ...
            if (roomId && chatRoomRef.current.includes(roomId)) {
                ...
                setMsgs((prevMsgs) => {
                    ...
                    console.log("SETTING MESSAGES TO:")
                    console.log(updated)
                    return updated || {};
                })
            }
    ...
    const _isUserOnline = (userId: string) => {
        return userId in _users && "connections" in (_users as any)[userId] && (_users as any)[userId].keys().length > 0
    }
     ...
     * @param uid 
     */
    const _createUserIfNotExists = (uid: string) => {
        if (!_userExists(uid)) {
    ...
    const _createChatIfNotExists = (uid1: string, uid2: string) => {
        ...
        get(child(ref(db), `chatRooms/${roomId}`)).then((snapshot) => {
            if (!snapshot.exists()) {
    ...
    const _sendMessage = (senderId: string, recipientId: string, message: string) => {
        ...
        _createUserIfNotExists(senderId)
        _createUserIfNotExists(recipientId)
